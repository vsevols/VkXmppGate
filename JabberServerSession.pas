...
uses
  IdContext, IdTCPClient, System.Classes, IdGlobal, GateGlobals,
  windows, System.Generics.Collections, GateXml, D7Compat;
...
type
  ...
  private
    ...
    procedure ProcessAuth(xml: TGateXmlNode);
    ...
    procedure ProcessIq(Node: TGateXmlNode);
    procedure ProcessIqGetDefault(Node: TGateXmlNode);
    procedure ProcessJabStream;
    procedure ProcessOnline(XML: TGateXmlParser);
    procedure ProcessResult(Node: TGateXmlNode);
  ...
  public
    ...
    procedure ProcessMessage(Node: TGateXmlNode);
...
procedure TJabberServerSession.ProcessAuth(xml: TGateXmlNode);
begin
...
try
  ...
  try
    ...
    if Pos('<?par', LowerCase(s))<>1  then
      par.xml:=Format('<STREAM>%s</STREAM>', [s])
      else
      par.xml:=s;
  except
  end;

   if (par.FirstChild<>nil) and (LowerCase(par.FirstChild.name)='auth') then
  begin
    ProcessAuth(par);
    exit;
  end;
...
procedure TJabberServerSession.ProcessIq(Node: TGateXmlNode);
var
  sFriendItems: string;
  sId: string;
begin
...
procedure TJabberServerSession.ProcessIqGetDefault(Node: TGateXmlNode);
var
  sId: string;
  xmlns: string;
begin
...
procedure TJabberServerSession.ProcessMessage(Node: TGateXmlNode);
var
  msg: TGateMessage;
begin
  // message event
...
procedure TJabberServerSession.ProcessOnline(XML: TGateXmlParser);
var
  Node : TGateXmlNode;
begin
...
procedure TJabberServerSession.ProcessResult(Node: TGateXmlNode);
begin
  try
