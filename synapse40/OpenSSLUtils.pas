...
RAND_load_file(ToD7PChar(fSeedFile), -1);
...
BIO_write_filename(PrivateKeyOut, ToD7PChar(fPrivateKeyFile));
PublicKeyOut := BIO_new(BIO_s_file());
BIO_write_filename(PublicKeyOut, D7PChar(fPublicKeyFile));
...
BIO_read_filename(certfile, ToD7PChar(FileName));
...
BIO_read_filename(p7file, ToD7PChar(Filename));
...
if BIO_write_filename(pkcs7file, ToD7PChar(Filename)) <= 0 then
...
if BIO_write_filename(contentfile, ToD7PChar(Filename)) <= 0 then
  raise EOpenSSL.Create('Error creating output file.');
...
BIO_read_filename(keyfile, ToD7PChar( ansistring(KeyFilename) ) );
if KeyPassword = '' then
  pw := nil
else
