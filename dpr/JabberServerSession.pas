...
begin
  ...
  //if this line uncommented - IndyTCPServer.Contexts.Count does not decrements
  // AV ?
  inherited;
end;
...
try
  ...
  try
    ...
    else if not bLineCutDone then
    begin // Empathy
      slXml.Delete(0);
      s:=slXml.Text;
      if Pos('<stream:', LowerCase(s))=1 then
      begin
        s:=s+'</stream:stream>';
        bLineCutDone:=true;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      //sServerName:='vkxmppsky.hopto.org'; // for Empathy
      if Pos('@', sLogin)<1 then
        sJid:=sLogin+'@'+sServerName
        else
          sJid:=sLogin;

      //Pidgin sometimes doesn't send resource
      if Node.getChildByName('bind').getChildByName('resource')<>nil then
        sJid:=sJid+'/'+Node.getChildByName('bind').getChildByName('resource').text;
...
var
  sXml: string;
begin       exit; // TODO: test with Empathy
  sXml:=
