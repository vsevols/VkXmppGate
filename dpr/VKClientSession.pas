...
//  if emoji.TableName<>'decimal_' then
  ...
  begin
    ...
    try
     ...
     { if at.getChildByName('type').text='wall' then
      ...
      if not bKnown then
        Result:=Result+CR+Format('++НЕИЗВЕСТНЫЙ ТИП ВЛОЖЕНИЯ %sim?sel=%s', [GetVkUrl, sChatUid]);
...
begin
  ...
  if sFullName='' then
    OnLog('QueryUserInfo1: ОШИБКА: не удалось получить данные для '+sUid);
end;
...
begin
  ...
  if (Node.getChildByName('title')<>nil)
    ...
    msg.sBody  := msg.sBody + MessageBodyTranslate(Node.getChildByName('body').text, true, true);
    try
